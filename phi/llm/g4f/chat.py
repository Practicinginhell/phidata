...
from g4f.client.stubs import ChatCompletionChunk
...
class G4F(LLM):
    ...
    g4f_client: Optional[Client] = None
    ...
    def response(self, messages: List[Message]) -> str:
        logger.debug(f"---------- {self.name} Response Start ----------")
    ...
    def response_stream(self, messages: List[Message]) -> Iterator[str]:
        ...
        for response in self.invoke_stream(messages=messages):
            ...
            # -*- Parse response
            # logger.info(f"G4F partial response: {response}")
            # logger.info(f"G4F partial response type: {type(response)}")

            response_message: Optional[ChatCompletionChunk] = response.choices[0].delta
            response_content = response_message.content if response_message else None
